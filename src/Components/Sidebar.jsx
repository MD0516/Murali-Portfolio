...
const Sidebar = () => {
  ...
  return (
    <>
      <motion.div key='sidebar-text' initial={{width: '280'}} style={{ width : toggleSidebar ? 70 : 280}} exit={{width: 70}} transition={{duration: 1, ease: 'easeInOut'}} className={`d-flex flex-column gap-3 align-items-center p-4 position-fixed sidebar `}  >
        ...
        <AnimatePresence> <motion.div initial= {{scale: 0.8, opacity: 0}} animate={{width: toggleSidebar ? 40 : 200, height: toggleSidebar ? 40 : 200, scale: 1, opacity: 1}} exit= {{scale: 0.8, opacity: 0}} transition={{ duration: 0.5, ease: 'easeInOut' }} className='rounded-circle bg-white d-flex justify-content-center align-items-center' ></motion.div> </AnimatePresence>
          { !toggleSidebar && (
            <div key='sidebar-text'  className='text-white text-center'>
              <h2>Murali Dharan</h2>
        ...
        <AnimatePresence >
          { !toggleSidebar &&(
            <motion.div key={toggleSidebar ? 'collapsed' : 'expanded'} initial={{ opacity:0,visibility: 'none' , y: -100 }} animate={{ opacity: 1, visibility: 'visible', y: 0 }} exit={{ opacity: 0,visibility: 'none', y:-100 }} transition={{ duration: 0.2 }} className='d-flex gap-3 custom-hover'>
        ...
        <div className='d-flex flex-column gap-3 nav' >
          <Link to='/explore' className={`${current === 'explore' || current === '' ? 'active' : ''} d-flex gap-2 justify-content-star`}>
            ...
            <AnimatePresence>{!toggleSidebar && (<motion.span key='Explore' initial={{ opacity: 0, visibility: 'none', x: -200 }} animate={{ opacity: 1,visibility: 'visible' , x: 0 }} exit={{ opacity: 0,  visibility: 'none', x: -200  }} transition={{ duration: 0.1 }}>Explore</motion.span>) }</AnimatePresence>
          </Link>
          <Link to='/skills' className={`${current === 'skills' ? 'active' : ''} d-flex gap-2 justify-content-start `}>
            ...
            <AnimatePresence>{!toggleSidebar && (<motion.span key='Skills' initial={{ opacity: 0, visibility: 'none', x: -200 }} animate={{ opacity: 1,visibility: 'visible' , x: 0 }} exit={{ opacity: 0,  visibility: 'none', x: -200  }} transition={{ duration: 0.2 }}>Skills</motion.span>)}</AnimatePresence>
            
          </Link>
          <Link to='/projects' className={`${current === 'projects' ? 'active' : ''} d-flex gap-2 justify-content-start`}>
            ...
            <AnimatePresence>{!toggleSidebar && (<motion.span key='Projects' initial={{ opacity: 0, visibility: 'none', x: -200 }} animate={{ opacity: 1,visibility: 'visible' , x: 0 }} exit={{ opacity: 0,  visibility: 'none', x: -200  }} transition={{ duration: 0.3 }}>Projects</motion.span>)}</AnimatePresence>
            
          </Link>
          <Link to='/about' className={`${current === 'about' ? 'active' : ''} d-flex gap-2 justify-content-start `}>
            ...
            <AnimatePresence>{!toggleSidebar && (<motion.span key='About' initial={{ opacity: 0, visibility: 'none', x: -200 }} animate={{ opacity: 1,visibility: 'visible' , x: 0 }} exit={{ opacity: 0,  visibility: 'none', x: -200  }} transition={{ duration: 0.4 }}>About</motion.span>)}</AnimatePresence>
            
          </Link>
          <Link to='/contact' className={`${current === 'contact' ? 'active' : ''} d-flex gap-2 justify-content-start`}>
            ...
            <AnimatePresence>{!toggleSidebar && (<motion.span key='Contact' initial={{ opacity: 0, visibility: 'none', x: -200 }} animate={{ opacity: 1,visibility: 'visible' , x: 0 }} exit={{ opacity: 0,  visibility: 'none', x: -200  }} transition={{ duration: 0.5 }}>Contact</motion.span>)}</AnimatePresence>              
          </Link>
        </div>
      </motion.div>

      <AnimatePresence>
        <motion.div key= 'sidebar-toggle' initial={{left: 260}} animate={{ rotate: toggleSidebar ? 180 : 0 , left : toggleSidebar ? 77 : 260}} exit={{left: 77}} transition={{ duration: 0.5 }} className={` d-flex justify-content-center align-items-center text-white position-fixed toggle-button`} onClick={handleSidebar} style={{top: 10}}>        
