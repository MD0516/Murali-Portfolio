...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('https://murali-portfolio.onrender.com/api/contact', {
        method: 'POST',
      ...
      <AnimatePresence>
        {animateNow && <motion.div initial={{opacity: 0, y: -400}} animate={{ opacity: 1, y: 0}} exit={{opacity: 0, y: -400}} transition={{ duration: .8, ease: 'easeInOut'}} className='container-fluid py-2 custom-color-responsiveness'>
          ...
          <div className='d-flex gap-3 '>
            ...
            <motion.button initial={{opacity: 0, x: xDir}} animate={{opacity: 1, x: 0}} exit={{opacity: 0, x: xDir}} transition={{duration: 1, ease: 'easeInOut'}} className='d-flex gap-1 fw-semibold py-2 px-4 contact-email rounded-3' onClick={handleCopy}>
                  ...
                  <motion.button type='submit' {...motionPropsPlus} className={`custom-bg col-12 py-2 submit-button ${isClicked ? 'submit-button-active' : ''}`} onClick={handleClicked} >
                    <div className='d-flex custom-bg'>
                      {isSubmitted ?(
                        ...
                        <motion.div animate={{rotate: 360}} transition={{ duration: .5, repeat: Infinity, ease: 'linear'}} className='loading-spinner d-inline-block custom-bg '></motion.div>
                      ):(
                        <>
                          Send Message
